...
class SantoDataset:
    ...
    def load(path):
        data = np.load(path)
        dset = SantoDataset()
        dset.states = list(data["states"])
        dset.actions = list(data["actions"])
        dset.scores = list(data["values"])
        return dset
